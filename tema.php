...
if (isset($_SESSION["usuario"])) {
    ...
    $sql = "SELECT id_usu FROM usuarios WHERE nickname = '$usuarioSesion'";
...
if (isset($_POST["borrar"])) {
    if (isset($_SESSION["usuario"])) {
        ...
        $sql = "DELETE FROM mensajes WHERE id_mensaje = $id_mensaje AND id_usu = $mi_id";
        $con->query($sql);
...
if (isset($_POST["mensaje"])) {
    if (isset($_SESSION["usuario"])) {
        ...
        $sql = "INSERT INTO mensajes (titulo_mensaje, mensaje, id_usu, id_tema) VALUES ('Respuesta', '$mensajeNuevo', $mi_id, $id_tema)";
        if ($con->query($sql)) {
            header("Location: tema.php?id=$id_tema");
            exit;
        } else {
...
$sql = "SELECT * FROM mensajes WHERE id_tema = $id_tema";
...
<div class="pagina-tema">
    ...
    <div class="mensajes">
        ...
        foreach ($mensajes as $mensaje) {
            ...
            $sql = "SELECT nickname, avatar, firma FROM usuarios WHERE id_usu = $id_usu";
            $resultadoUsuario = $con->query($sql);
            $nickname = "Usuario";
            $avatar = "";
            $firma = "";
            ...
            <div class="mensaje">
                <div class="autor">
                    <?php if ($avatar != NULL && $avatar != "") { ?>
                        <img src="avatars/<?php echo htmlspecialchars($avatar, ENT_QUOTES, "UTF-8"); ?>" alt="" class="avatar-img" width="64" height="64" loading="lazy">
                    <?php } else { ?>
                ...
                <p class="mensaje-texto"><?php echo $mensaje["mensaje"]; ?></p>
                <?php
                /* SOLO MOSTRAMOS BORRAR SI EL MENSAJE ES NUESTRO */
                if (isset($_SESSION["usuario"])) {
                    if ($mensaje["id_usu"] == $mi_id) {
                ?>
                        <form action="tema.php?id=<?php echo $id_tema; ?>" method="POST" class="borrar-mensaje">
                            <input type="hidden" name="id_mensaje" value="<?php echo $mensaje["id_mensaje"]; ?>">
    ...
    ?>
        <div class="respuesta">
            <h2>Responder</h2>
            <form action="tema.php?id=<?php echo $id_tema; ?>" method="POST">
                <div class="editor-cabecera">
                    <label for="respuesta-mensaje">Tu mensaje</label>
                    <div class="editor-botones">
                        <button type="button" onclick="ponerEmoji('respuesta-mensaje', '😊')">😊</button>
                        <button type="button" onclick="ponerEmoji('respuesta-mensaje', '❤️')">❤️</button>
                        <button type="button" onclick="ponerEmoji('respuesta-mensaje', '👍')">👍</button>
                        <button type="button" onclick="ponerEmoji('respuesta-mensaje', '😡')">😡</button>
                        <button type="button" onclick="ponerEmoji('respuesta-mensaje', '😭')">😭</button>
                        <button type="button" onclick="ponerEmoji('respuesta-mensaje', '👾')">👾</button>
                ...
                <textarea id="respuesta-mensaje" name="mensaje" placeholder="Escribe tu respuesta..." required></textarea>
    ...
    ?>
        <div class="cta">
            <p>Inicia sesión para participar en esta conversación.</p>
            <a href="login.php" class="b1">Iniciar sesión</a>
        </div>
    <?php
    }
    ?>
